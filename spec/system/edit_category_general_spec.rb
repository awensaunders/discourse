...
describe "Edit Category General", type: :system do
  ...
  context "when changing text color" do
    it "displays an error when the hex code is invalid" do
      ...
      expect(form.field("text_color")).to have_errors(I18n.t("js.category.foreground_color_validations.non_hexdecimal"))

      form.field("text_color").component.find("input.hex-input").fill_in(with: "")
      category_page.save_settings
      pause_test
      expect(form.field("text_color")).to have_errors(I18n.t("js.category.foreground_color_validations.cant_be_empty"))

      form.field("text_color").component.find("input.hex-input").fill_in(with: "A")
      category_page.save_settings
      expect(form.field("text_color")).to have_errors(I18n.t("js.category.foreground_color_validations.incorrect_length"))

      form.field("text_color").component.find("input.hex-input").fill_in(with: "AACCBB1")
      category_page.save_settings
      expect(form.field("text_color")).to have_errors(I18n.t("js.category.foreground_color_validations.incorrect_length"))
    end
