...
RSpec.configure do |config|
  ...
  class TestCurrentUserProvider < Auth::DefaultCurrentUserProvider
    def log_on_user(user, session, cookies, opts = {})
      # Ensure using the main session as `session` sometimes is a server session
      cookies.request.session[:current_user_id] = user.id
      super
    end

    def log_off_user(session, cookies)
      # Ensure using the main session as `session` sometimes is a server session
      cookies.request.session[:current_user_id] = nil
      super
    end
  end
